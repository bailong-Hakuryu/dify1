    ...
    def _handle_plugin_daemon_error(self, error_type: str, message: str):
        ...
        match error_type:
            ...
            case PluginInvokeError.__name__:
                current_type = PluginInvokeError.__name__
                current_message = message
                
                import os
                max_unwrap_depth = int(os.getenv("PLUGIN_DAEMON_MAX_UNWRAP_DEPTH", "5"))
                for _ in range(max_unwrap_depth):
                    try:
                        error_object = json.loads(current_message)
                    except Exception:
                        break
                    ...
                    inner_type = error_object.get("error_type")
                    inner_message = error_object.get("message")
                    
                    if not isinstance(inner_type, str) or not isinstance(inner_message, str):
                        break
                        
                    current_message = inner_message
                    current_type = inner_type
                    
                    if current_type != PluginInvokeError.__name__:
                        break

                invoke_error_type = current_type
                error_object = {"error_type": current_type, "message": current_message}
                match invoke_error_type:
                    ...
                    case _:
                        raise PluginInvokeError(description=current_message)
