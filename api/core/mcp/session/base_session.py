    ...
    def send_request[T: BaseModel](
        ...
        try:
            ...
            if request_read_timeout_seconds is not None:
                max_timeout = float(request_read_timeout_seconds.total_seconds())
            elif self._session_read_timeout_seconds is not None:
                max_timeout = float(self._session_read_timeout_seconds.total_seconds())

            start_time = time.time()
            while True:
                elapsed = time.time() - start_time
                if elapsed >= max_timeout:
                    raise TimeoutError(f"Request {request_id} timed out after {max_timeout} seconds")
                poll_interval = min(DEFAULT_RESPONSE_READ_TIMEOUT, max_timeout - elapsed)
                try:
                    response_or_error = response_queue.get(timeout=poll_interval)
                    break
                except queue.Empty:
                    self.check_receiver_status()
                    continue
